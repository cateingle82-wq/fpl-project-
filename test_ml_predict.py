...
def make_histories():
    """player 101: one game played (round 9). player 102: five games
    (established). player 103: no history at all (brand new)."""
    def gw_row(round_, pts, mins):
        return {
...
def test_fixtures_by_team_week_handles_blank_and_double():
    ...
    team1 = fixmap[1]
    assert team1[0] == [(2, True)], team1[0]              # GW10: home vs team2
    assert 1 not in team1, "GW11 (week offset 1) should be a blank — no entry"
    assert set(team1[2]) == {(3, False), (4, True)}, team1[2]  # GW12: double
...
def test_base_features_games_so_far_correct():
    ...
    print("2. player_base_features: games_so_far matches real history count  OK")
...
def test_zero_history_player_has_nan_rolling_features():
    ...
    print("3. a player with zero games has NaN rolling features (not 0)      OK")
...
def test_predict_cold_start_only_includes_cold_start_players(monkeypatch):
    ...
    print("4. predict_cold_start only returns players under the games threshold  OK")
...
def test_predict_cold_start_blank_week_is_zero():
    ...
    print("5. a blank gameweek predicts exactly 0, not a guess                OK")
...
def test_predict_cold_start_double_week_sums_both_fixtures():
    ...
    print("6. a double gameweek sums two independent per-fixture predictions  OK")
...
def test_no_model_returns_empty_dataframe(monkeypatch):
    ...
    assert preds.empty
    print("7. no trained model -> empty DataFrame, not a crash               OK")
